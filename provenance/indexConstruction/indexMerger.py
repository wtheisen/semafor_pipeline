...
def mergeIndexShards(shardFolderList,emptyIndexPath,outDir):
    ivfs = []
    bar = progressbar.ProgressBar()
    outDir = os.path.abspath(outDir)
    fileutil.make_sure_path_exists(outDir)
    outPath = os.path.join(outDir,'index.index')
    for indexFolder in bar(shardFolderList):
        ...
        indexFile = os.path.join(indexFolder)
    ...
    ivfDataStr = os.path.join(outDir,'index.ivfdata')
    ...
    print('Vector data: ', ivf_vector.data())
    ntotal = invlists.merge_from(ivf_vector.data(), ivf_vector.size())
    ...
    print('written!')
    #
    # print('Merging DBM databases...')
    #
    # newIDMap = dbm.open(os.path.join(outDir,'IDMap_db.dbm'),'c')
    # for indexFolder in indexList:
    #     dbmpath = os.path.join(indexFolder,'IDMap_db')
    #     dbtmp = dbm.open(dbmpath,'r')
    #     bar = progressbar.ProgressBar()
    #     for k in bar(dbtmp):
    #         newIDMap[k] = dbtmp[k]
    #     newIDMap.sync()
    # print('Merging On-Disk Meta Ararys...')
