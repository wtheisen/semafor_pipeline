...
def _filterGeomConsMatches(matches, queryImgKeypoints, trainImgKeypoints, refMatchCount, displMatchThreshold):
    ...
    for i in range(refMatchCount - 1):
        ...
        if len(currentJs) > 0:
            consistentMatches = list(map(lambda j: _filterIJMatches(matches, i, j, queryPoints, trainPoints, displMatchThreshold), currentJs))
...
def match(keypoints1, descriptions1, image1, keypoints2, descriptions2, image2, 
    ...
    if len(firstAndSecondMatches) > 0:
        try:
            ...
            goodMatches = _filterGeomConsMatches(goodMatches, keypoints1, keypoints2, refMatchCount, displMatchThreshold)
        except:
